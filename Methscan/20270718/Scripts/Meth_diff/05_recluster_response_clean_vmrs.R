...
valid_groups <- c("IR", "NR")
valid_variants <- c("threshold005", "threshold002", "threshold001")
valid_stages <- c("before", "after")
if (
  length(args) != 3 ||
    !args[[1]] %in% valid_groups ||
    !args[[2]] %in% valid_variants ||
    !args[[3]] %in% valid_stages
) {
  stop(
    "Usage: Rscript 05_recluster_response_clean_vmrs.R ",
    "<IR|NR> <threshold005|threshold002|threshold001> <before|after>",
    call. = FALSE
  )
}
response_group <- args[[1]]
variant <- args[[2]]
stage <- args[[3]]
analysis_id <- paste(response_group, variant, stage, sep = "_")
...
umap_threads <- as.integer(Sys.getenv("METHSCAN_UMAP_THREADS", unset = "32"))
...
if (is.na(umap_threads) || umap_threads < 1) {
  stop("METHSCAN_UMAP_THREADS must be >= 1.", call. = FALSE)
}
...
before_matrix_root <- Sys.getenv(
  "BEFORE_MATRIX_ROOT",
  unset = file.path(base_dir, "result/response_VMR_before_individual")
)
after_matrix_root <- Sys.getenv(
  "AFTER_MATRIX_ROOT",
  unset = Sys.getenv(
    "RESPONSE_MATRIX_ROOT",
    unset = file.path(base_dir, "result/response_VMR_remove_individual")
  )
)
matrix_root <- if (stage == "before") before_matrix_root else after_matrix_root
input_file <- file.path(
  matrix_root, response_group, variant, "VMR_matrix",
...
output_root <- Sys.getenv(
  "RESPONSE_CLUSTER_ROOT",
  unset = file.path(base_dir, "result/response_VMR_reclustering")
)
output_dir <- file.path(output_root, response_group, variant, stage)
plot_dir <- file.path(output_dir, "plots")

dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(plot_dir, showWarnings = FALSE, recursive = TRUE)

message("Group: ", response_group)
...
save(pca, file = file.path(output_dir, paste0(analysis_id, "_PCA.RData")))
write.csv(
  pca_tbl,
  file.path(output_dir, paste0(analysis_id, "_PCA_coordinates.csv")),
  row.names = FALSE
)
...
write.csv(
  umap_tbl,
  file.path(output_dir, paste0(analysis_id, "_UMAP_coordinates.csv")),
  row.names = FALSE
)
...
neighbor_graph <- neighbor_graph_edges %>%
  select(from, to, weight) %>%
  igraph::graph_from_data_frame(directed = FALSE)

leiden_resolution_arg <- if (
...
clust_obj <- do.call(
  ...
  c(
    list(graph = neighbor_graph),
    stats::setNames(list(leiden_resolution), leiden_resolution_arg)
  )
)
...
write.csv(
  clust_tbl,
  file.path(output_dir, paste0(analysis_id, "_annotation.csv")),
  row.names = FALSE
)

observed_responses <- unique(stats::na.omit(clust_tbl$response))
if (
  length(observed_responses) != 1 ||
    observed_responses[[1]] != response_group
) {
  stop(
    "Matrix/annotation response mismatch: expected ", response_group,
    "; observed ", paste(observed_responses, collapse = ","),
    call. = FALSE
  )
}
...
pca_plot <- pca_tbl %>%
  ...
  coord_fixed() +
  labs(title = paste(response_group, variant, stage, "PCA"))
...
cell_type_plot <- plot_tbl %>%
  ...
  coord_fixed() +
  labs(title = paste(response_group, variant, stage, "UMAP by cell type"))

sample_plot <- plot_tbl %>%
  ggplot(aes(UMAP1, UMAP2, color = sample)) +
  geom_point(size = 0.4, alpha = 0.8) +
  coord_fixed() +
  labs(title = paste(response_group, variant, stage, "UMAP by sample"))

cluster_plot <- plot_tbl %>%
  ggplot(aes(UMAP1, UMAP2, color = leiden_cluster)) +
  geom_point(size = 0.4, alpha = 0.8) +
  coord_fixed() +
  labs(title = paste(response_group, variant, stage, "UMAP by Leiden cluster"))

ggsave(file.path(plot_dir, paste0(analysis_id, "_PCA.png")), pca_plot, width = 8, height = 6, dpi = 300)
ggsave(file.path(plot_dir, paste0(analysis_id, "_UMAP_by_cell_type.png")), cell_type_plot, width = 10, height = 8, dpi = 300)
ggsave(file.path(plot_dir, paste0(analysis_id, "_UMAP_by_sample.png")), sample_plot, width = 10, height = 8, dpi = 300)
ggsave(file.path(plot_dir, paste0(analysis_id, "_UMAP_by_leiden.png")), cluster_plot, width = 10, height = 8, dpi = 300)

run_metadata <- tibble(
  key = c(
    "group", "variant", "stage", "input_matrix", "input_cells", "matched_cells", "input_VMRs",
    "removed_all_NA_VMRs", "removed_zero_variance_VMRs",
    "n_pcs", "umap_n_neighbors", "umap_min_dist", "umap_threads", "umap_seed",
    "leiden_resolution", "reference_annotation"
  ),
  value = as.character(c(
    response_group, variant, stage, input_file, input_cell_count, nrow(meth_mtx), ncol(meth_mtx),
    feature_qc_summary$removed_all_NA,
    feature_qc_summary$removed_zero_variance,
    n_pcs, umap_n_neighbors, umap_min_dist, umap_threads, random_seed,
...
message("Done: ", response_group, " ", variant, " ", stage)
