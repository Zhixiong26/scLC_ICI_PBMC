...
prepare_groups() {
    ...
    if [ "${FORCE_GROUPS}" = "1" ] || [ "${group_count}" -eq 0 ] || [ ! -f "${summary_csv}" ]; then
        ...
        if ! Rscript "${SCRIPT_DIR}/02_generate_cell_groups.R" "${annotation_csv}" "${groups_dir}" "${data_dir}"; then
...
run_threshold() {
    ...
    groups_dir="${SCRIPT_DIR}/cell_groups_${threshold}"
    output_root="${SCRIPT_DIR}/result/DMR_results_${threshold}"
    log_root="${SCRIPT_DIR}/logs/${threshold}"
