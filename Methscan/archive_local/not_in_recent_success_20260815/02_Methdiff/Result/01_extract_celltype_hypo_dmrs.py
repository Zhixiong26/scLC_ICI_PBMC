#!/usr/bin/env python3
"""Extract cell-type hypomethylated DMRs from sample-wise MethSCAn comparisons.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output directory (default: celltype_hypo_DMRs_diff0p30 beside the input "
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for line_number, row in enumerate(reader, start=1):
            ...
            yield HypoRecord(
                ...
                source_file=str(path.relative_to(result_dir)),
...
def discover_inputs(
    result_dir: Path, selected_samples: set[str]
) -> tuple[dict[str, list[tuple[Path, str, str]]], dict[str, dict[str, set[str]]]]:
    inputs: DefaultDict[str, list[tuple[Path, str, str]]] = defaultdict(list)
    ...
    def register(path: Path, expected_sample: str | None = None) -> None:
        ...
        inputs[sample].append((path, left_cell, right_cell))
    ...
    for sample_dir in sorted(result_dir.glob("*_sample_celltype")):
        ...
        for path in sorted(sample_dir.glob("*_DMRs.bed")):
            register(path, expected_sample=directory_sample)
...
def process_sample(
    sample: str,
    sample_inputs: Sequence[tuple[Path, str, str]],
    ...
    for path, left_cell, right_cell in sample_inputs:
    ...
    with (sample_root / "input_summary.tsv").open(
        ...
        writer = csv.writer(handle, delimiter="\t", lineterminator="\n")
        writer.writerow(["sample", "input_files", "input_DMR_rows", "passing_rows"])
        writer.writerow([sample, len(sample_inputs), audit["source_rows"], passing_rows])

    print(
        f"[{sample}] completed: {audit['source_rows']} input rows, "
        f"{passing_rows} passing rows",
        flush=True,
    )
    return summary_rows
...
def run(args: argparse.Namespace) -> Path:
    ...
    inputs, opponents = discover_inputs(result_dir, selected_samples)
    ...
    if output_dir is None:
        ...
        output_dir = default_output_parent / "celltype_hypo_DMRs_diff0p30"
    else:
