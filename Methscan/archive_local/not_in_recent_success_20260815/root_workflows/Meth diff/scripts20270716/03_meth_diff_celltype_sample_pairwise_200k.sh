#!/bin/bash
################################################################################
# Run within-response sample-pairwise MethScan comparisons separately for each
# cell type. With 15 cell types and 10 pairs per response, this generates up to
# 300 comparisons; pairs below MIN_CELLS are retained in the summary but skipped.
#
...
prepare_groups() {
    ...
    if [ "${FORCE_GROUPS}" = "1" ] || [ "${group_count}" -eq 0 ] || [ ! -f "${summary_csv}" ]; then
        ...
        if ! Rscript "${SCRIPT_DIR}/04_generate_celltype_sample_pairwise_groups.R" "${annotation_csv}" "${groups_dir}" "${data_dir}"; then
...
run_threshold() {
    ...
    groups_dir="${SCRIPT_DIR}/cell_groups_celltype_sample_pairwise_${threshold}"
    output_root="${SCRIPT_DIR}/result/celltype_sample_pairwise/DMR_results_${threshold}"
    log_root="${SCRIPT_DIR}/logs/celltype_sample_pairwise/${threshold}"
