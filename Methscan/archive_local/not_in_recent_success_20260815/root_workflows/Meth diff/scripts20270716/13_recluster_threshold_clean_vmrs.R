...
analysis_stage <- Sys.getenv("METHSCAN_STAGE", unset = "after")
if (!analysis_stage %in% c("before", "after")) {
  stop("METHSCAN_STAGE must be before or after.", call. = FALSE)
}
feature_label <- if (analysis_stage == "before") "All VMRs" else "Clean VMRs"
...
pca_plot <- pca_tbl %>%
  ...
  coord_fixed() +
  labs(title = paste0("PCA: ", variant, " ", feature_label))
...
run_metadata <- tibble(
  key = c(
    "variant", "stage", "input_matrix", "input_cells", "matched_cells", "input_VMRs",
  ...
  value = as.character(c(
    variant, analysis_stage, input_file, input_cell_count, nrow(meth_mtx), ncol(meth_mtx),
