#!/usr/bin/env Rscript
# Generate within-response sample-pairwise MethScan groups for every cell type.
...
cell_types <- sort(unique(annot$cell_type[eligible]))

for (cell_type in cell_types) {
  safe_cell_type <- sanitize(cell_type)
  ...
  for (response in names(sample_sets)) {
    ...
    for (pair in sample_pairs) {
      ...
      summary_rows <- append(summary_rows, list(write_comparison(
        annot,
        paste0("6_", response, "_within_celltype_sample_pairwise"),
        paste0(safe_cell_type, "_", sample_a, "_vs_", sample_b),
        eligible & annot$cell_type == cell_type & annot$sample == sample_a,
        eligible & annot$cell_type == cell_type & annot$sample == sample_b,
        paste(cell_type, sample_a, sep = ":"),
        paste(cell_type, sample_b, sep = ":")
      )))
    }
  }
}
...
cat("Eligible cells by cell type and sample:\n")
print(annot %>% filter(eligible) %>% count(cell_type, sample), n = Inf)
