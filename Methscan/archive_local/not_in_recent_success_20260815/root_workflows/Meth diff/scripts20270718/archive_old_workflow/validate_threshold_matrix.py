#!/usr/bin/env python3
"""Stream-validate a threshold-specific MethSCAn matrix."""
...
def read_clean_ids(path: Path) -> list[str]:
    ...
    if not ids:
        raise ValueError(f"Clean VMR BED is empty: {path}")
    if len(ids) != len(set(ids)):
        raise ValueError(f"Clean VMR BED contains duplicate IDs: {path}")
    return ids
...
def main() -> None:
    ...
    with gzip.open(matrix_path, "rt", newline="") as handle:
        ...
        missing = sorted(set(clean_ids) - set(matrix_ids))
        extra = sorted(set(matrix_ids) - set(clean_ids))
        if missing or extra:
            raise ValueError(
                "Matrix VMR header differs from Clean VMR BED; "
                f"missing={len(missing)} extra={len(extra)}"
            )
        order_matches = matrix_ids == clean_ids
    ...
    with output_path.open("w") as output:
        ...
        output.write(f"clean_BED_VMRs\t{len(clean_ids)}\n")
