...
| Scanpy cell type | 细胞数 | 占杂色细胞比例 |
|---|---:|---:|
| `CD8_T_cells` | 37 | 22.42% |
| `NK_cells` | 34 | 20.61% |
| `CD4_T_cells` | 28 | 16.97% |
| `Naive_CD4_T_cells` | 26 | 15.76% |
| `B_cells` | 13 | 7.88% |
| `cDCs` | 10 | 6.06% |
| `Treg_cells` | 6 | 3.64% |
| `Plasma_cells` | 5 | 3.03% |
| `pDCs` | 3 | 1.82% |
| `Gamma_delta_T_cells` | 2 | 1.21% |
| `Cycling_cells` | 1 | 0.61% |
| **合计** | **165** | **100.00%** |

各类型百分比以 165 为分母并独立保留两位小数，因此逐行显示值相加可能有
0.01 个百分点的舍入误差。

其中数量最多的是 `CD8_T_cells`、`NK_cells`、`CD4_T_cells` 和
`Naive_CD4_T_cells`。四者合计 125 个，占全部红色杂色细胞的 **75.76%**；
因此该区域主要由 T/NK 细胞构成，而不是单一的新细胞群。
