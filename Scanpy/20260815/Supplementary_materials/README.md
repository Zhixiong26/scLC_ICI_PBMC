...
环境必须实际包含 Scrublet API；仅安装 Scanpy 但没有 `scrublet` 时，`02_integration.py` 会明确拒绝运行。

## R 与 DoubletFinder 依赖

DoubletFinder 分支的 `02_integration.py` 会通过 `Rscript` 逐样本调用 `03_doubletfinder.R`。执行该分支的运行环境需要：
