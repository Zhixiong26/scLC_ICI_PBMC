...
# Step 09d: add a fifth Top200 heatmap without replacing Steps 08, 09, 09b,
# or 09c. Standardize every DMR across observed cells and clip the actual
# plotted/saved Z-score matrix to [-1, 1].
...
process_sample() {
    ...
    figure_dir="$analysis_root/figures_top200_DMRwise_zscore_clipped_minus1_to1"
    ...
    if [[ -e "$figure_dir" ]]; then
        if [[ "$PLOT_OVERWRITE" == 1 ]]; then
            ...
            echo "[$short OVERWRITE] removed previous clip-1 Z-score heatmap"
        elif [[ -s "$figure_dir/heatmap_summary.tsv" ]]; then
            echo "[$short REUSE] Top200 Z-score heatmap clipped to [-1, 1]"
            return 0
        else
            echo "ERROR: $short partial clip-1 Z-score output exists: $figure_dir" >&2
            return 1
        fi
    fi

    echo "[$short RUN] Top200 DMR-wise Z-score heatmap clipped to [-1, 1]"
    ...
    echo "[$short OK] clip-1 Z-score heatmap: $figure_dir"
}
...
[[ "$failures" -eq 0 ]] || die "$failures sample(s) failed clip-1 Z-score plotting"
echo "[ALL SAMPLES OK] independent Top200 DMR-wise Z-score heatmaps clipped to [-1, 1] complete"
