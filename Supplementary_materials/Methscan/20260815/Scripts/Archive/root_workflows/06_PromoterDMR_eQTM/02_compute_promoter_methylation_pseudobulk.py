#!/usr/bin/env python3
"""Compute bidirectional promoter-DMR DNA pseudobulk from deduplicated cov."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--overlap",
        type=Path,
        default=RESULT_ROOT / "01_promoter_DMR_map" / "promoter_DMR_overlap.tsv.gz",
    )
    parser.add_argument(
        "--candidates",
        type=Path,
    ...
    parser.add_argument("--output-dir", type=Path, default=RESULT_ROOT / "02_DNA_pseudobulk")
...
def load_interval_sets(
    ...
    for row in overlap.itertuples(index=False):
        ...
        if (cell_type, gene) not in pairs:
            continue
        direction = str(row.dmr_direction)
        if direction not in {"IR_hypo", "IR_hyper"}:
            raise ValueError(f"Unexpected DMR direction: {direction}")
        region = f"{direction}_DMR_supported"
        for token in str(row.overlap_intervals).split(","):
            chrom, coordinates = token.split(":", 1)
            start_text, end_text = coordinates.split("-", 1)
            supported_raw[(cell_type, gene, region, chrom)].append(
    ...
    supported_by_pair: dict[
        tuple[str, str, str], list[tuple[str, int, int]]
    ] = defaultdict(list)
    for (cell_type, gene, region, chrom), intervals in supported_raw.items():
        for start, end in merge_intervals(intervals):
            supported_by_pair[(cell_type, gene, region)].append(
                (chrom, start, end)
            )
    ...
    for cell_type, gene in sorted(pairs):
        ...
        supported_count = 0
        for region in (
            "IR_hypo_DMR_supported",
            "IR_hyper_DMR_supported",
        ):
            supported_intervals = supported_by_pair.get(
                (cell_type, gene, region), []
            )
            supported_count += len(supported_intervals)
            for chrom, start, end in supported_intervals:
                by_type[cell_type][chrom].append(
                    (start, end, gene, region)
                )
        if not full_intervals or supported_count == 0:
...
def main() -> None:
    ...
    regions_by_pair: dict[tuple[str, str], set[str]] = defaultdict(set)
    for cell_type, chrom_map in intervals.items():
        for records in chrom_map.values():
            for _start, _end, gene, region in records:
                regions_by_pair[(cell_type, gene)].add(region)
    ...
    for candidate in candidates.itertuples(index=False):
        ...
        region_order = {
            "full_promoter": 0,
            "IR_hypo_DMR_supported": 1,
            "IR_hyper_DMR_supported": 2,
        }
        regions = sorted(
            regions_by_pair[(cell_type, gene)],
            key=lambda value: region_order[value],
        )
    ...
    parameters = {
        ...
        "IR_hypo_DMR_supported": (
            "covered CpGs in IR-hypo DMR/promoter overlap intervals"
        ),
        "IR_hyper_DMR_supported": (
            "covered CpGs in IR-hyper DMR/promoter overlap intervals"
        ),
